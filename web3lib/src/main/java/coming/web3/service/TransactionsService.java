...
import coming.web3.enity.repository.EthereumNetworkRepositoryType;
import coming.web3.enity.repository.TokenRepository;
import coming.web3.enity.repository.Transaction;
import coming.web3.enity.repository.TransactionLocalSource;
import coming.web3.enity.repository.TransactionMeta;
import coming.web3.enity.repository.TransactionType;
...
public class TransactionsService
{
    ...
    private Pair<Integer, Boolean> getNextChainIndex(int currentIndex, boolean nftCheck, List<Long> filters)
    {
        ...
        while (true)
        {
            ...
            EthereumNetworkInfo info = ethereumNetworkRepository.getNetworkByChain(filters.get(currentIndex));
    ...
    private void setNextTransferCheck(long chainId, boolean isNft)
    {
        ...
        if (filters.contains(chainId))
        {
            ...
            EthereumNetworkInfo info = ethereumNetworkRepository.getNetworkByChain(chainId);
    ...
    private void readTokenMoves(long chainId, boolean isNFT)
    {
        ...
        final EthereumNetworkInfo info = ethereumNetworkRepository.getNetworkByChain(chainId);
        if (isNFT)
        {
    ...
    private void checkTransactionQueue()
    {
        ...
        if (fetchTransactionDisposable == null)
        {
            ...
            if (t != null)
            {
                ...
                EthereumNetworkInfo network = ethereumNetworkRepository.getNetworkByChain(t.tokenInfo.chainId);
    ...
    private void checkPendingTransactions()
    {
        ...
        for (final Transaction tx : pendingTxs)
        {
            ...
            web3j.ethGetTransactionByHash(tx.hash).sendAsync().thenAccept(txDetails -> {
                org.web3j.protocol.core.methods.response.Transaction fetchedTx =
                        txDetails.getTransaction().get(); //try to read the transaction data
